...
import { getUser, setToken } from "../utils/utils";
const Login = () => {
  ...
  const handleSubmit = (e) => {
    ...
    const response = fetch(`${import.meta.env.VITE_SERVER_HOST}/auth/login`, {
      method: "POST",
      credentials: "include",
      headers: {
    ...
    <div className="flex justify-center items-center h-screen">
      <div className="flex flex-col mt-[300px] items-center gap-5 sm:flex-row sm:mt-0 justify-between  w-screen m-40  "  >
        ...
        <div className=" scale-75 p-10= sm:scale-100 flex flex-col justify-center border-2 bg-white rounded-2xl shadow-2xl items-center sm:w-[500px] sm:h-[450px]">
          ...
          <NavLink to="/signup">
            <p className="text-orange-400 mt-4 flex gap-1 items-center">Don't have an account ? Signup Here <LogIn/> </p> 
          </NavLink>
          <p className="text-orange-400">OR</p>
          <NavLink to="/reset-password">
            <p className="text-orange-400  flex gap-1 items-center">Forgot your password ? Reset Here  </p> 
          </NavLink>  
          </div>
        </div>
      </div>
